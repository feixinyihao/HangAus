...
@interface SetFlavorViewController ()
@property(nonatomic,strong)NSArray*flavoArray;
@property(nonatomic,strong)NSMutableArray*isSelectedArray;
@property(nonatomic,strong)NSMutableDictionary*propDict;
@end

@implementation SetFlavorViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.flavoArray=@[@"Cook Level",@"In Batter",@"Crumbed",@"Salt",@"Chicken Salt",@"Vinega",@"Sauce",@"Flour"];
    [self setupArray];
...
-(void)setupArray{
    for (int i=1; i<=pow(2, self.flavoArray.count); i++) {
        if (((i-1)&i)==0) {
            if ((self.flavorProp&i)==i) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.textLabel.text=self.flavoArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (isSelected) {
        self.flavorProp=self.flavorProp-pow(2, indexPath.row);
        [self.propDict setObject:@(self.flavorProp) forKey:@"prop"];
    }else{
        self.flavorProp=self.flavorProp+pow(2, indexPath.row);
        [self.propDict setObject:@(self.flavorProp) forKey:@"prop"];
    }
